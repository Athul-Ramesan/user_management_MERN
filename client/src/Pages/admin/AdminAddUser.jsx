...
const AdminAddUser = () => {
    ...
    const handleAddUserSubmit = (event) => {
        ...
        try {
            if (!stringRegex.test(name.trim())) {
                ...
                dispatch(signupUserAction({ name, email, password },navigate,setErrorMessage,isAdmin))
            }
        } catch (err) {
            console.error(err);
            setShowError(true);
    ...
    return (
        <div className='w-full flex flex-col justify-center bg-slate-600 items-center min-h-screen'>
            <h3 className='font-bold text-xl m-10 '>Create User</h3>
            <form onSubmit={handleAddUserSubmit} className='flex flex-col justify-center items-center w-2/5 bg-white relative border-[1px] border-black py-5'>
